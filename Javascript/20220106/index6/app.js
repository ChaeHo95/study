...
novel.split('').filter(word => !specialCharacters.includes(word) && word != ' '&& word != '\n').forEach( element => {
    const text = element.toLowerCase()
    if(classified[text] > 0){
        classified[text]++
    }else{
        classified[text] = 1
    }
...
for(let element in classified){
    alphabetFrequency.push({[element]:classified[element]})
}

alphabetFrequency.sort((a,b) => a.values - b.values)
